  ...
  <section className="py-4 sm:py-5 bg-white">
    <div className="container mx-auto px-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {steps.map((s) => (
          <div key={s.step} className="flex flex-col items-center text-center p-3 sm:p-4 rounded-xl bg-white border border-gray-200 hover:bg-gray-50 transition-all shadow-sm">
            <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center mb-2">
              <s.icon className="h-4 w-4 text-gray-700" />
            </div>
            <span className="text-[10px] font-bold text-gray-600 uppercase tracking-wider mb-0.5">Step {s.step}</span>
            <h3 className="font-display font-bold text-sm text-gray-900 mb-0.5">{s.title}</h3>
            <p className="text-[11px] text-gray-600 font-body leading-snug">{s.desc}</p>
          </div>
        ))}
      </div>
    </div>
